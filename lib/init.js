...
async function runGuidanceNonInteractive(root, flags) {
  ...
  // Explicit harness: init always targets portable project AGENTS.md.
  const args = [
    process.execPath,
    "furanku-skills",
    "inject",
...
/**
 * Ask (interactively) whether to replace a blocking CLAUDE.md with the symlink.
 * Non-interactive callers rely on --force instead.
 *
 * @param {import("readline").Interface | null} rl
 * @param {ReturnType<typeof inspectAgentsSetup>} status
 * @param {{ force?: boolean }} flags
 * @returns {Promise<boolean>}
 */
async function resolveForceForClaude(rl, status, flags) {
  if (flags.force) return true;
  if (!status.claudeBlocks) return false;
  if (!rl || !process.stdin.isTTY) return false;

  print();
  print(style.warn(`${status.claudeBlockReason}.`));
  print(
    style.dim(
      "  Setup wants CLAUDE.md → AGENTS.md so Claude Code and other agents share one file."
    )
  );
  if (!status.claude.isSymlink) {
    print(
      style.dim(
        "  Replacing removes the existing CLAUDE.md (contents are not merged into AGENTS.md)."
      )
    );
  }
  print();
  return confirm(rl, "Replace CLAUDE.md with a symlink to AGENTS.md?", true);
}

function printAgentsResult(result) {
  for (const m of result.messages) {
    const leftUnchanged = /left unchanged/i.test(m);
    if (result.dryRun) print(style.info(`[dry-run] ${m}`));
    else if (leftUnchanged) print(style.warn(m));
    else print(style.ok(m));
  }
}
...
async function agentsMdCommand(tokens) {
  ...
  print(style.bullet(`${style.dim("root")} ${style.path(flags.root)}`));
  if (status.complete) {
    print(style.ok(`Already set up: ${style.bold("AGENTS.md")} + ${style.bold("CLAUDE.md")} → AGENTS.md`));
    return;
  }

  const interactive = process.stdin.isTTY && !flags.yes && !flags.dryRun;
  ...
  if (interactive) {
    ...
    try {
      ...
      force = await resolveForceForClaude(rl, status, flags);
    } finally {
      rl.close();
    }
  ...
  if (!force && status.claudeBlocks && (flags.yes || flags.dryRun)) {
    print(
      style.warn(
        `${status.claudeBlockReason}. Pass --force to replace with a symlink to AGENTS.md.`
      )
    );
  }
...
async function interactiveInit(flags) {
  ...
  print(style.sectionTitle("Project setup wizard"));
  ...
  try {
    ...
    if (status.complete) {
      ...
      if (go) {
        const force = await resolveForceForClaude(rl, status, flags);
        const result = createAgentsMd({
          root,
          dryRun: flags.dryRun,
          force,
        ...
        printAgentsResult(result);
        summary.push(
          force && status.claudeBlocks
            ? "agents-md: updated (replaced CLAUDE.md)"
            : "agents-md: updated"
        );
      } else {
...
async function nonInteractiveInit(flags) {
  ...
  print(style.sectionTitle("Non-interactive init"));
  print(style.bullet(`${style.dim("root")} ${style.path(root)}`));
  print();
