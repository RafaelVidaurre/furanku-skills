...
const { createRl, confirm, chooseIndex, multiSelect } = require("./prompt");
...
${style.dim("Options:")}
  ...
  --skills all|<id,id>      Install skills from ${SKILLS_SOURCE}
  --no-skills               Skip skill install
  --skills-global, --global Install skills with -g (user-level)
...
async function interactiveInit(flags) {
  ...
  try {
    ...
    const available = listCollectionSkills();
    let skillsChoice = flags.skills;
    if (skillsChoice === false) {
      print(style.dim("Skipped (--no-skills)."));
      summary.push("skills: skipped");
    } else {
      print(
        style.dim(
          `Source: ${SKILLS_SOURCE}  (${available.length} skills)`
        )
      );
      const want =
        skillsChoice != null
          ? true
          : await confirm(rl, "Install skills from this collection?", true);
      if (!want) {
        print(style.dim("Skipped."));
        summary.push("skills: skipped");
      } else {
        let selected = skillsChoice;
        if (selected == null) {
          const pick = await chooseIndex(
            rl,
            "Which skills?",
            ["All skills", "Pick specific skills"],
            0
          );
          if (pick === 0) {
            selected = "all";
            ...
            selected = await multiSelect(
              rl,
              "Select skills",
              available.map((id) => ({ id, label: "" }))
            );
            if (selected.length === 0) {
              print(style.warn("No skills selected; skipping install."));
              selected = false;
            }
          }
        }
        if (selected !== false) {
          if (Array.isArray(selected)) validateSkillIds(selected);
          let global = flags.skillsGlobal;
          if (!flags.skillsGlobal && skillsChoice == null) {
            const scope = await chooseIndex(
              rl,
              "Install scope",
              ["Project (this repo)", "Global (user-level)"],
              0
            );
            global = scope === 1;
          }
          ...
          if (!result.dryRun) {
            ...
            if (!result.ok) {
              ...
              summary.push(
                `skills: ${selected === "all" ? "all" : selected.join(",")}${global ? " (global)" : ""}`
              );
            }
          } else {
            summary.push(
              `skills: dry-run ${selected === "all" ? "all" : selected.join(",")}`
            );
          }
        } else {
          summary.push("skills: skipped");
