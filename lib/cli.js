...
${style.dim("Examples:")}
  ...
  furanku-skills guidance-composer inject --ids simplest-current --mode inline --harness agents --yes
...
function menuEntries(argv) {
  return [
    ...
    },
    {
      ...
      run: async () => {
        print(style.dim("Bye."));
        // TTY stdin stays referenced after raw-mode menus and keeps the
        // event loop alive — exit explicitly.
        process.exit(0);
      },
    },
  ];
...
async function main(argv = process.argv) {
  try {
    ...
    // Interactive menus resume stdin; release so the process can end.
    releaseStdin();
  }
}
