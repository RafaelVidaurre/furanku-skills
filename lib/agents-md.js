...
function inspectAgentsSetup(root, fsLike = fs) {
  ...
  /** True when CLAUDE.md exists but is not the desired AGENTS.md symlink. */
  const claudeBlocks =
    claude.exists && !(claude.isSymlink && claude.pointsToAgents);

  let claudeBlockReason = null;
  if (claudeBlocks) {
    if (claude.isSymlink) {
      claudeBlockReason = `${CLAUDE_NAME} is a symlink to ${claude.target || "?"} (not ${AGENTS_NAME})`;
    } else {
      claudeBlockReason = `${CLAUDE_NAME} already exists as a regular file`;
    }
  }
  ...
  if (fsLike.existsSync(claudePath)) {
    ...
    if (st.isSymbolicLink()) {
      ...
      if (resolved === path.resolve(agentsPath)) {
        ...
        skipped.push(CLAUDE_NAME);
        messages.push(
          `${CLAUDE_NAME} is a symlink to ${target || "?"} (not ${AGENTS_NAME}); left unchanged`
        );
      }
    } else if (force) {
      if (!dryRun) {
      ...
      skipped.push(CLAUDE_NAME);
      messages.push(
        `${CLAUDE_NAME} already exists as a regular file; left unchanged`
      );
    }
  } else {
    if (!dryRun) {
