...
4. **Jev classifies completed work retrospectively.** Identify *task outcomes within* every informative session whose model and effort can be verified, including sessions outside model routing. Use Jev for domain and feedback classification; use tests, artifacts, user acceptance, and visual review for quality evidence. Link routed launches to worker sessions separately when evaluating the selector. Never turn an `unknown` into an average score.
...
The unit is one requested work outcome, direct or delegated, rather than an entire chat. Record the session ID, catalog/rubric versions, task profile, actual model/effort, artifact references, acceptance checks, user follow-ups, elapsed work, and provider spend/quota category. When a routing decision exists, record its decision ID and verified worker link as additional policy-evaluation fields. Session history is read from its source at review time; the routing journal holds links and metadata, not transcript copies.
...
2. **Outcome collection:** mine direct and delegated Codex, Claude Code, and Grok sessions by verified model and effort; obtain task-level artifact/user evidence and report missingness. Keep worker links for the separate routing-policy evaluation. Run Jev assessment in shadow mode and manually audit a stratified sample, especially art and critical tasks.
