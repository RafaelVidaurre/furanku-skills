...
def main():
    ...
    if args.inventory.parent != private or args.output.parent != private:
        parser.error(f"Inventory and output must be directly inside {private}")
    if args.inventory.is_symlink() or args.output.is_symlink() or private.is_symlink():
    ...
    results, counts = analyze(events, inventory)
