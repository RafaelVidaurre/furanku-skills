...
Account-scoped launchers use the active session's measured account; a machine-wide provider account must not override them. When one candidate's launch surface genuinely always bills a fixed account, register that identity under `accounts` and opt that candidate in with `quota_account`; only then does a mismatch refuse the launch. A bare `accounts` entry is a registry value, not a provider-wide routing rule.
