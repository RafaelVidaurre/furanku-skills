...
For `/model-routing logs status|on|off|tail`, read [Private routing journal](references/logging.md) and run the matching `routing_log.py` command. `off` is the machine-wide opt-out for future events.
