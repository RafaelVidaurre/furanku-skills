...
- **Runtime** is where the code runs: `server` (runs remotely: a service process, or code a hosted platform runs such as serverless functions or on-chain contracts), `client` (a page, desktop, mobile, or game app a person uses), `fullstack` (one app whose own code runs both as a server and as the app people use, like server-side rendering or a game that builds its player app and headless servers; the client/server check never applies to it), `shared` (a library compiled into more than one runtime), `cli` (a command run by a person or a script), `build` (runs only while building or developing), `none` (not executable: content, docs).
