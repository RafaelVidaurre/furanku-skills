...
- `loaded_by` (only when it applies): the ids of components, or of externals in `system.externals`, that load this one through something other than an import: compiled to Wasm and loaded by a page, called through FFI, generated code, a plugin the host discovers, code an outside engine or indexer runs. An unknown id fails the build. The import scan cannot see these links, so without them the map says the component ships in nothing. Cite the build script or doc that shows the link in `evidence`.
