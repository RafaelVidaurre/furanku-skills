...
def test_pnpm_monorepo_units_and_resolution(tmp_path):
    ...
    assert by_path["tools/check.mjs"] == {"path": "tools/check.mjs", "lang": "js", "loc": 4, "role": "source", "unit": "tools", "changes": 1, "provenance": "authored"}
...
def test_build_config_and_generated_or_data_files_are_told_apart():
    assert scan.file_role("apps/web/vite.config.ts") == "build"
    assert scan.file_role("apps/web/tailwind.config.js") == "build"
    assert scan.file_role("apps/web/src/config.ts") == "source"
    assert scan.file_role("apps/web/src/app.test.ts") == "test"
    assert scan.file_provenance("sdk/src/generated/contract.ts", ["export const x = 1;"]) == "generated"
    assert scan.file_provenance("sdk/src/types.ts", ["// @generated by codegen", "export type A = 1;"]) == "generated"
    table = ["export const IDS = ["] + [f"  {i}," for i in range(2500)] + ["];"]
    assert scan.file_provenance("subgraph/src/ids.ts", table) == "data"
    calls = ["export const types = new Map();"] + [f"types.set('{i}', {i % 3});" for i in range(2500)]
    assert scan.file_provenance("subgraph/src/types-1.ts", calls) == "data"
    code = (HERE / "scan.py").read_text().splitlines() * 2  # real, varied code well over the size threshold
    assert len(code) >= 2000 and scan.file_provenance("src/big.py", code) == "authored"


def test_gdscript_and_solidity_imports_resolve_by_path(tmp_path):
    root = make_repo(tmp_path, extra={
        "game/project.godot": '[application]\nconfig/name="Arena"\nrun/main_scene="res://scenes/main.tscn"\n',
        "game/scenes/main.tscn": '[gd_scene format=3]\n[ext_resource type="Script" path="res://scripts/main.gd" id="1"]\n',
        "game/scripts/main.gd": 'extends Node\nconst Player = preload("res://scripts/player.gd")\nvar hud = load("res://scenes/hud.tscn")\n',
        "game/scripts/player.gd": 'extends Actor\nclass_name Player\n',
        "game/scripts/actor.gd": 'extends CharacterBody2D\nclass_name Actor\n',
        "game/scenes/hud.tscn": '[ext_resource type="Script" path="res://scripts/hud.gd" id="1"]\n',
        "game/scripts/hud.gd": 'extends "res://scripts/actor.gd"\n',
        "game/tests/test_player.gd": 'extends GutTest\nconst P = preload("res://scripts/player.gd")\n',
        "contracts/package.json": '{"name": "contracts"}\n',
        "contracts/src/Game.sol": 'pragma solidity ^0.8.0;\nimport "./Token.sol";\nimport {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";\n',
        "contracts/src/Token.sol": 'pragma solidity ^0.8.0;\n',
    })
    doc = scan.scan(root, now=NOW)
    edges = {(e["from"], e["to"]) for e in doc["edges"]}
    assert ("game/scripts/main.gd", "game/scripts/player.gd") in edges  # preload by res:// path
    assert ("game/scripts/main.gd", "game/scripts/hud.gd") in edges  # loading a scene depends on its script
    assert ("game/scripts/player.gd", "game/scripts/actor.gd") in edges  # extends a class_name
    assert ("game/scripts/hud.gd", "game/scripts/actor.gd") in edges  # extends a script path
    assert ("contracts/src/Game.sol", "contracts/src/Token.sol") in edges
    assert "@openzeppelin/contracts" in {x["name"] for x in doc["externals"]}
    units = {u["path"]: u for u in doc["units"]}
    assert units["game"]["kind"] == "godot" and units["game"]["name"] == "Arena" and units["game"]["hints"]["executable"]
...
def test_a_build_config_importing_a_library_scans(tmp_path):
    root = make_repo(tmp_path, extra={
        ...
        "web/package.json": '{"name": "web", "private": true, "scripts": {"dev": "vite"}}\n',
        "web/vite.config.ts": "import { defineConfig } from 'vite';\nexport default defineConfig({});\n",
        "web/src/main.ts": "export const app = 1;\n",
    })
    doc = scan.scan(root, now=NOW)
    roles = {f["path"]: f["role"] for f in doc["files"]}
