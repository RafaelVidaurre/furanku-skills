...
BUILD_CONFIG = re.compile(
    r"^(?:[\w.-]+\.config|\.?eslintrc|babel\.config|webpack\.config|rollup\.config|vite\.config|vitest\.config|jest\.config"
    r"|tailwind\.config|postcss\.config|next\.config|nuxt\.config|svelte\.config|astro\.config|playwright\.config"
    r"|hardhat\.config|metro\.config|tsup\.config|esbuild\.config|build|setup|noxfile|conanfile)\.[cm]?[jt]sx?$"
    r"|^(?:setup|noxfile|conanfile)\.py$|^build\.rs$"
)


def file_role(path: str) -> str:
    """'test' for files that exist to exercise other code, 'build' for build configuration, else 'source'."""
    parts = path.split("/")
    if any(part in TEST_DIRS for part in parts[:-1]):
        return "test"
    if TEST_BASENAME.search(parts[-1]):
        return "test"
    return "build" if BUILD_CONFIG.search(parts[-1]) else "source"
...
def file_provenance(path: str, lines: list[str]) -> str:
    """'generated' for tool-written files, 'data' for large literal tables encoded as source, else 'authored'."""
...
def scan(repo_root: Path, ref: str = "HEAD", now: str | None = None) -> dict:
    ...
    for path in sorted(scanner.source):
        ...
        files.append({"path": path, "lang": lang, "loc": len(lines), "role": file_role(path), "unit": unit.id if unit else None,
                      "changes": changes.get(path, 0), "provenance": file_provenance(path, lines)})
        if lang in ("ts", "js"):
