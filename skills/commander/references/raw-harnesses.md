...
Raw mode is a supervised job runner built from one-shot agent commands, isolated workspaces, operating-system processes, and Commander's private ledger. The examples below are discovery seeds; reusable machine configuration supplies the normal launch recipe.
...
Reuse configured Captain and Sergeant recipes. For the primary candidates, confirm only that each harness command exists:

```sh
command -v <captain-harness>
command -v <sergeant-harness>
```

Use `<cli> --version`, a local authentication-status command, or focused help only when the configured recipe is missing information or the installed command shape has changed. These checks must not invoke a model.

Raw mode is ready when the selected commands are present and the configured recipe defines non-blocking launch, stable process or session identity, bounded output capture, completion detection, and stop or recovery. The first real dispatch establishes current model access, permissions, quota, and lifecycle behavior; failures follow the normal candidate or mechanism recovery path.
...
| --- | --- | --- |
| Codex | `codex exec -C <workspace> -s <configured-sandbox> -o <report> -` | `-m <model> -c 'model_reasoning_effort="<effort>"'` |
| Claude Code | `claude -p --permission-mode <configured-mode> <prompt>` | `--model <model> --effort <effort>` |
| Grok | `grok --prompt-file <brief> --permission-mode <configured-mode>` | `--model <model> --reasoning-effort <effort>` |
...
Write `brief.md` before launch. The configured invocation or post-exit extraction owns `report.md`; when the harness emits only a final answer on stdout, treat the completed stdout log as the report. Use a wrapper that writes `exit-code` only after the harness exits. A POSIX seed is:
...
' sh "$job_dir" <configured-harness-command> <configured-arguments...> \
  >/dev/null 2>&1 &

printf '%s\n' "$!" >"$job_dir/pid"
```

Treat this wrapper as a schematic: substitute an argument-safe invocation and account for harnesses whose prompt flag does not read stdin. Prefer a configured durable-agent or terminal/session interface when the environment provides one.

Record the job and PID or session in the ledger immediately. Use a harness session or terminal/process-group identity expected to control the worker's descendants. If a real stop or recovery reveals that the identity is insufficient, classify a mechanism failure and preserve the workspace before changing the recipe. Return control after confirming the process survived startup long enough to create observable state; delivery continues in the background.
...
Use the configured resume or session command when it preserves the same worker context and workspace. Otherwise create a new dispatch directory and a continuation brief containing the prior report, relevant diagnostic tail, decisions, and current workspace state.
...
- its concurrency allowance under the resolved policy;
- the configured Sergeant candidates;
- the concrete channel or file through which it reports to Commander.

The Captain launches child jobs in namespaced dispatch directories, observes them inside its own background process, and emits one consolidated completion report. Commander reserves the Captain's allowance and monitors its report rather than writing the Captain's child state or taking over live children. If the Captain is lost, Commander inventories every child PID/session and workspace before appointing a replacement Captain.
