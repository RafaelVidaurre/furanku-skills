...
**What it does:** compiles a routing brief (research-backed candidates, your preferences, live quota), lets the configured selector (Jev or the spawning agent) choose, then gate-checks the pick: disabled candidates, missing features, agents the mechanism cannot launch, and exhausted quota are refused with reasons, and unknown quota needs an explicit sign-off.
